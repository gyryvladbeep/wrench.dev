...
test.describe("Cron Expression Parser", () => {
  ...
  test("пресет 'Every 15 minutes' — конкретная ветка описания для '*/N'", async () => {
  ...
  test("выражение '0 */6 * * *' — ветка 'Every day at H:00' срабатывает раньше ветки '*/N hours' (реальное поведение)", async () => {
    // ВАЖНО: несмотря на то, что пресет называется "Every 6 hours", при min==="0"
    // первой всегда срабатывает ветка "Every day at {hours}:00" — ветка
    // "every N hours" в describeCron() при min==="0" недостижима. Тест
    // фиксирует РЕАЛЬНОЕ поведение страницы; это находка для отдельного
    // обсуждения, не баг в тесте.
    ...
    await expect(tool.description).toHaveText("Every day at 0, 6, 12, 18:00");
  });
});
