 ...
 * без реального аккаунта тут нет смысла: гость с сентября 2026 видит
 * на /workbench WorkbenchGuestPreview (локальный demo-холст без
 * сохранения, см. header-nav.spec.ts и components/workbench/
 * WorkbenchGuestPreview.tsx), а этот файл проверяет именно то, что
 * гостю недоступно принципиально — persist/rename/delete/лимиты/
 * публичный шаринг настоящего, сохраняемого рабочего стола. Один
 * test.describe.serial с одним page на всю группу — по той же причине,
 * что и в auth-flow.spec.ts: нам нужен один и тот же рабочий стол от
 * шага к шагу, а не свежий пустой аккаунт на каждый
 * test().
 *
...
test.describe.serial("Workbench: жизненный цикл рабочего стола", () => {
  ...
  test("карточки инструментов можно перетаскивать по свободному холсту", async () => {
    ...
    // ИЗВЕСТНЫЙ БАГ ПРОДУКТА (не теста!) — шире, чем казалось на первый
    // взгляд. Раньше здесь подозревали только сдвиг avoidPanel() (см. тест
    // каскада выше), но прямое измерение через boundingBox()/elementFromPoint
    // на отдельном диагностическом стенде (WorkbenchCanvas с тем же
    // дефолтным набором из 6 инструментов, без Supabase) показало более
    // общую причину: CASCADE_ROW_GAP в lib/workbench-layout.ts — это 260px,
    // а реальная высота карточки "по содержимому" у большинства инструментов
    // заметно больше (у JSON Formatter — ~530px, у Regex Tester — ~640px) —
    // так что карточка ряда 0 почти всегда налезает на карточку под ней в
    // том же столбце. С z-index это не смертельно (последний в tool_slugs
    // рисуется поверх, так что каждая карточка остаётся кликабельной САМА
    // ПО СЕБЕ ровно в своих границах), НО у HTTP Status Codes (использовался
    // здесь раньше как источник драга) высота карточки — около 2744px(!):
    // судя по всему, инструмент рендерит целиком статичную таблицу всех HTTP
    // статус-кодов без внутреннего скролла. Такая карточка не просто
    // перекрывает соседа под собой в том же столбце — она физически
    // утаскивает мышь Playwright далеко за пределы обычного вьюпорта при
    // попытке навестись на её центр, из-за чего сам жест dragTo() не
    // срабатывает ожидаемо (даже когда таймаута на перехвате клика уже нет).
    // Берём пару из ряда 1 (оба — под панелью авто-сдвига avoidPanel не
    // попадают, см. тест каскада) и с нормальной, некоаномальной высотой —
    // ни Regex Tester, ни XPath Generator ни с кем в своём ряду по X не
    // пересекаются (между столбцами 32px зазора, CASCADE_COL_GAP шире
    // CANVAS_CARD_WIDTH), и оба подтверждённо "выигрывают" hit-test в
    // собственном центре. Сама аномальная высота HTTP Status Codes и общая
    // теснота CASCADE_ROW_GAP — отдельные, непочиненные продуктовые баги,
    // не theste; чинить их — в WorkbenchCanvas.tsx/lib/workbench-layout.ts
    // и в реализации инструмента HTTP Status Codes, с визуальной проверкой.
    await wb.dragCardOnto("Regex Tester", "XPath Generator");
