...
test.describe.serial("Workbench: жизненный цикл рабочего стола", () => {
  ...
  test("карточки инструментов можно перетаскивать по свободному холсту", async () => {
    ...
    // ИЗВЕСТНЫЙ БАГ ПРОДУКТА (не теста!), обнаружен здесь: раньше тянули
    // before[0]/before[last] — универсально при любом порядке. Но
    // before[0] это JSON Formatter (колонка 0, ряд 0, x=20 y=20) — именно
    // та карточка, которую avoidPanel() в WorkbenchCanvas.tsx подвигает
    // вниз из-под плавающей панели (см. предыдущий тест каскада). Сдвиг
    // там — на фиксированные avoidTopLeft.height+16, а не на целый
    // CASCADE_ROW_GAP, так что сдвинутая карточка физически накладывается
    // на Regex Tester (та же колонка 0, ряд 1) под ней — его текстовое
    // поле перехватывает клики, и dragTo() не может даже нажать на
    // исходную карточку, тест падает по таймауту. Это реальное наложение
    // карточек друг на друга в самом продукте, не только в тесте — если
    // у пользователя первый добавленный инструмент оказался под панелью
    // (частый случай, панель всегда в левом верхнем углу — там же, где и
    // "ячейка 0" каскада), его карточка может перекрыть карточку под ней
    // и потерять кликабельность. Требует отдельного исправления в
    // WorkbenchCanvas.tsx/lib/workbench-layout.ts с визуальной проверкой —
    // здесь временно тянем заведомо не перекрытую пару (колонка 2, куда
    // панель такой ширины физически не достаёт ни в одном ряду), чтобы
    // проверить сам механизм drag & drop, не упираясь в этот баг.
    await wb.dragCardOnto("HTTP Status Codes", "CSS Selector Generator");
