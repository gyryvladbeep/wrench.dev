...
//
// 420, а не изначальные 340 — у части инструментов (JSON Formatter и
// похожие с двумя колонками "ввод/результат") внутренняя раскладка
// переключается на md:grid-cols-2 по ШИРИНЕ БРАУЗЕРА, а не по ширине
// самой карточки (Tailwind-брейкпоинты завязаны на viewport, а не на
// контейнер) — на десктопе это правило срабатывает всегда, независимо
// от того, насколько узкая карточка его показывает. При 340px это
// сжимало обе колонки до нечитаемых ~150px. 420px не убирает причину
// (для этого нужны CSS container queries — самих инструментов такой
// раскладкой под тридцать штук, это отдельная более крупная правка),
// но даёт двум колонкам вменяемую ширину, если пользователь такой
